...
// The goproxy's main purpose is to be a daemon connecting the docker daemon
// (remote API) and the custom Minecraft server (cuberite using lua scripts).
...
//
// As a daemon, goproxy listens for events from the docker daemon and sends
// them to the cuberite server. It also listens for requests from the
